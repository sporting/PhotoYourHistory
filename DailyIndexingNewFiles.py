...
def DailyIndexingNewImageFile():   
    ...
    #if len(dirs)<=0:
    #    dbh.insertDirIfNotExist((DefaultMonitorDir,True))
    dirs = dbh.getMonitorDirs()
    ...
    for d in dirs:
        ...
        if lastParserUtc:
            dirTime =  os.path.getmtime(d['DIR'])
            dirUtcTime = tzh.getUTCTime(dirTime)
            dirUtcTimestamp = dth.dateTimeToTimestamp(dirUtcTime)
            lastParserUtcTimestamp = dth.dateTimeToTimestamp(lastParserUtc)
            if !(dirUtcTimestamp>= lastParserUtcTimestamp):
                print(d['DIR']+' pass!')
                continue
